# Year 3 Project – Distributed Shared-Buffer (DSB) NoC Router
...
- X–Y routing in a 2D-mesh network.  
...
## 🚦 3-Stage Pipeline (detailed)

### Stage 1 — Input / Decode / Input Buffering
...
### Stage 2 — Arbiter / Input→Middle Memory (XB1 + MM_WR)
- Arbitration decides which input goes to which MM, resolves arrival conflicts, and writes granted flits into selected MMs via a 5×N switch. MM occupancy bitmaps and fixed/circular priority logic are used for selection.

### Stage 3 — Middle Memory→Output (MM_RD + XB2 + Output Link)
...
- **2×2 Mesh simulation:** Compared a network built from traditional 3-stage IBR routers vs. the implemented DSB routers. Observed that the DSB router yields lower average packet delay under many multi-directional traffic patterns due to more flexible internal buffering and allocation.
...
1. R. S. Ramanujam, V. Soteriou, B. Lin, and L.-S. Peh, “Design of a High-Throughput Distributed Shared-Buffer NoC Router,” Proc. 4th ACM/IEEE Int. Symp. Networks-on-Chip (NoCs), Grenoble, France, May 3–6, 2010..
